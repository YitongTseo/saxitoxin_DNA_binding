...
def remove_files(lst_tuples) -> None:
    ...
    for failed_structs, prefix_str, suffix_str in failed_list:
        ...
        stage_regex = re.compile(rf"{prefix_str}.*_.*{suffix_str}.(inp|out|job).*")
        ...
        if stage_ok:
            ...
            disposables_jobo = find_disposables(
                job_out_files, failed_structs, stage_regex
            )
            disposables_jobe = find_disposables(
                job_err_files, failed_structs, stage_regex
            )
        ...
        if not stage_ok:
            ...
            remaining_out_files = [
                f for f in sorted(glob.glob("*_*.out*")) if stage_regex.search(f)
            ]
            if remaining_out_files:
                ...
                if not os.path.exists(nname):
                    print(
                        f"Run failed. Linking {random_file} to {nname} for user inspection."
                    )
